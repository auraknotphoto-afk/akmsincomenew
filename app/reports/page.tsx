            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white flex items-center gap-3">
                  <span className="text-3xl">📊</span> Income Reports
                </h1>
                <p className="text-indigo-300 text-sm mt-1">Analyze your income by period</p>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={exportToCSV} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 text-white font-medium hover:bg-white/20 transition-colors">
                <FileSpreadsheet className="w-5 h-5" /> CSV
              </button>
              <button onClick={exportToPDF} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold hover:shadow-lg hover:shadow-indigo-500/25 transition-all">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
              ...
              <>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="date" value={customDateRange.startDate} onChange={(e) => setCustomDateRange({ ...customDateRange, startDate: e.target.value })} className="w-full pl-11 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-indigo-400" />
                    <input type="date" value={customDateRange.endDate} onChange={(e) => setCustomDateRange({ ...customDateRange, endDate: e.target.value })} className="w-full pl-11 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                  </div>
                </div>
              </>
            )}
            <div className={selectedPeriod !== 'custom' ? 'md:col-span-3 md:max-w-xs' : ''}>
              <label className="block text-sm font-medium text-indigo-300 mb-2">Category</label>
              <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="text-indigo-300 mt-4">Loading report data...</p>
          </div>
        ) : (
          <>
            {/* Summary Cards */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-gradient-to-br from-emerald-600/20 to-emerald-600/5 backdrop-blur border border-emerald-500/20 rounded-2xl p-6">
                <p className="text-emerald-300 text-sm font-medium">Total Jobs</p>
                <p className="text-3xl font-bold text-white mt-2">{totals.jobs}</p>
              </div>
              <div className="bg-gradient-to-br from-blue-600/20 to-blue-600/5 backdrop-blur border border-blue-500/20 rounded-2xl p-6">
                <p className="text-blue-300 text-sm font-medium">Total Income</p>
                <p className="text-3xl font-bold text-white mt-2">₹{totals.income.toLocaleString('en-IN')}</p>
              </div>
              <div className="bg-gradient-to-br from-cyan-600/20 to-cyan-600/5 backdrop-blur border border-cyan-500/20 rounded-2xl p-6">
                <p className="text-cyan-300 text-sm font-medium">Amount Received</p>
                <p className="text-3xl font-bold text-white mt-2">₹{totals.paid.toLocaleString('en-IN')}</p>
              </div>
              <div className="bg-gradient-to-br from-amber-600/20 to-amber-600/5 backdrop-blur border border-amber-500/20 rounded-2xl p-6">
                <p className="text-amber-300 text-sm font-medium">Pending Amount</p>
                <p className="text-3xl font-bold text-white mt-2">₹{totals.pending.toLocaleString('en-IN')}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              {categorySummaries.map((cat) => (
                <div key={cat.category} className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-6 hover:border-indigo-500/50 transition-all">
              ...
              <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl overflow-hidden mb-8">
                <div className="p-6 border-b border-white/10">
                  <h2 className="text-lg font-bold text-white flex items-center gap-2">
                    <PieChart className="w-5 h-5 text-indigo-400" /> Monthly Breakdown
                  </h2>
                </div>
            ...
            <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl overflow-hidden">
              <div className="p-6 border-b border-white/10">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <FileText className="w-5 h-5 text-indigo-400" /> Detailed Transactions ({jobs.length})
                </h2>
              </div>
              {jobs.length === 0 ? (
                <div className="text-center py-12">
                  <span className="text-5xl">📭</span>
                  <h3 className="text-lg font-bold text-white mt-4">No Transactions Found</h3>
                  <p className="text-indigo-300 mt-2">Try selecting a different time period</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
